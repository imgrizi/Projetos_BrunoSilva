...
    'img/Slide-Water-Bottle-Mockup_orange.png'   // Sobe da pasta 'js' para aceder a 'img'
  ...
  if (!slider || !bottle) {
    console.error('Slider ou imagem da garrafa não encontrados!');
    return;
  }
  ...
  slider.addEventListener('input', () => {
    ...
    if (value < 33) {
      bottle.src = imagens[0];
    } else if (value < 66) {
      bottle.src = imagens[1];
    } else {
      bottle.src = imagens[2];
    }
  });
});
...
const productVideo = document.querySelector('.product-video-overlay');
let productVideoPlayed = false;

if (productSection && productVideo) {
  const productObserver = new IntersectionObserver((entries) => {
    entries.forEach(entry => {
      if (entry.isIntersecting && !productVideoPlayed) {
        productVideoPlayed = true;
        productVideo.classList.add('visible');
        productVideo.play();
        productVideo.addEventListener('ended', () => {
          productVideo.classList.remove('visible');
        });
      }
    });
  }, { threshold: 3 });
